...
import (
	...
	. "github.com/fj9140/myproxy/regretable"
)
...
func TestRegretableReader(t *testing.T) {
	...
	if string(s) != word {
		t.Errorf("Uncommitted read is gone, [%d,%d] actual '%v' excepted '%v'\n", len(s), len(word), string(s), word)
	}
}
...
func TestRegretableEmptyRead(t *testing.T) {
	...
	if string(s) != word {
		t.Error("Uncommitted read is gone, actual:", string(s), "excepted:", word, "err:", err)
	}
}
...
func TestRegretableRead(t *testing.T) {
	buf := new(bytes.Buffer)
	mb := NewRegretableReader(buf)
	word := "12345678"
	buf.WriteString(word)
